...
class ScrollWatcher {
	...
	scrollWatcherConstructor(items) {
		if (items.length) {
			// this.scrollWatcherLogging(`Слежу за обьектами (${items.length})...`);
			...
			// this.scrollWatcherLogging("Нет обьектов для слежения!");
		}
	}
	...
	getScrollWatcherConfig(paramsWatch) {
		...
		if (document.querySelector(paramsWatch.root)) {
			...
			// this.scrollWatcherLogging(`Нет родительского элемента ${paramsWatch.root} на странице`);
		}
		// Відступ спрацьовування
		configWatcher.rootMargin = paramsWatch.margin;
		if (paramsWatch.margin.indexOf('px') < 0 && paramsWatch.margin.indexOf('%') < 0) {
			this.scrollWatcherLogging(`Предупреждение, настройку data-watch-margin нужно задавать в PX или %`);
			return
		}
	...
	scrollWatcherIntersecting(entry, targetElement) {
		if (entry.isIntersecting) {
			...
			//this.scrollWatcherLogging(`Есть ${targetElement.classList}, додав клас _watcher-view`);
		} else {
			// Не бачимо об'єкт
			// Забираємо клас
			targetElement.classList.contains('_watcher-view') ? targetElement.classList.remove('_watcher-view') : null;
			//this.scrollWatcherLogging(`Нет ${targetElement.classList}, прибрав клас _watcher-view`);
		}
	}
	// Функція відключення стеження за об'єктом
	scrollWatcherOff(targetElement, observer) {
		observer.unobserve(targetElement);
		// this.scrollWatcherLogging(`Стоп ${targetElement.classList}`);
	}
